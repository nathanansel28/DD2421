...
def mlParams(
    ...
    C = len(classes)    # no. of unique classes
    N, d = np.shape(X)  # no. of data points & features

    if W is None:
        W = np.ones((N, 1)) / float(N)  # assume equal weights

    mu = np.zeros((C, d))
    sigma = np.zeros((C, d, d))

    counts = np.zeros(C)            # creates a zero array of no. of classes
    np.add.at(mu, inverse, X)       # add data point to respective class mean
    # count no.of samples belonging to each class
    np.add.at(counts, inverse, 1)
    mu /= counts[:, None]           # divide by class count to get average

    for idx, k in enumerate(classes):
        mask = labels == k          # select data points belonging to class k
        W_k = W[mask]               # extract feature vectors for class k
        X_k = X[mask]               # extract weight vector for class k

        # sum of weights for class k
        W_k_sum = np.sum(W_k)
        mu[idx] = np.sum(W_k * X_k, axis=0) / \
            W_k_sum       # compute weighted mean
...
def computePrior(
    ...
    Npts = labels.shape[0]                              # get no. of data points
    if W is None:
        W = np.ones((Npts, 1))/Npts                     # assign equal weights
    else:
        # check if W has same no.of elements as labels
        assert (W.shape[0] == Npts)
    classes = np.unique(labels)                         # distinct class labels
    Nclasses = np.size(classes)                         # no. unique classes

    # zero arr to store the priors
    prior = np.zeros((Nclasses, 1))
    # compute weighted priors for each class
    for idx, k in enumerate(classes):
        mask = labels == k
...
def classifyBayes(
    ...
    arrays = [
        (
            ...
            - 1/2 * np.einsum('ij,jk,ik->i',
                              (X - mu[k]), np.linalg.inv(sigma[k]), (X - mu[k]))
            + np.log(prior[k])
...
def trainBoost(
    ...
    for i_iter in range(0, T):
        ...
        # the latest classifier predicts labels for all pts in X
        vote = classifiers[-1].classify(X)

        # find misclassified pts
        ...
        alpha = 0.5 * (np.log((1 - error) / error))     # compute importance
        alphas.append(alpha)
...
def classifyBoost(
    ...
    if Ncomps == 1:
        ...
        for t in range(Ncomps):
            # Get predictions from classifier t
